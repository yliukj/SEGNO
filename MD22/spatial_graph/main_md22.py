...
from md22.dataset import MD22Dataset
from n_body_system.model import GNN, Baseline, Linear, EGNN_vel, Linear_dynamics, RF_vel, GMN, SEGNO
import os
...
parser.add_argument('--nf', type=int, default=64, metavar='N',
                    help='hidden dim')
parser.add_argument('--model', type=str, default='segno', metavar='N',
...
parser.add_argument('--mol', type=str, default='nhme',
                    help='Name of the molecule.')
parser.add_argument('--data_dir', type=str, default='spatial_graph/md22',
                    help='Data directory.')
...
def main():
    ...
    dataset_train = MD22Dataset(partition='train', max_samples=args.max_training_samples, data_dir=args.data_dir,
                                molecule_type=args.mol, delta_frame=args.delta_frame)
    loader_train = torch.utils.data.DataLoader(dataset_train, batch_size=args.batch_size, shuffle=True, drop_last=True,
                                               num_workers=8)

    dataset_val = MD22Dataset(partition='val', max_samples=2000, data_dir=args.data_dir,
                                molecule_type=args.mol, delta_frame=args.delta_frame)
    loader_val = torch.utils.data.DataLoader(dataset_val, batch_size=args.batch_size, shuffle=False, drop_last=False,
                                             num_workers=8)

    dataset_test = MD22Dataset(partition='test', max_samples=2000, data_dir=args.data_dir,
                                molecule_type=args.mol, delta_frame=args.delta_frame)
    ...
    if args.model == 'gnn':
        ...
        model = SEGNO(in_node_nf=2, in_edge_nf=2 + 3, hidden_nf=args.nf, device=device, n_layers=args.n_layers,
        ...
        model = GMN(in_node_nf=2, in_edge_nf=2 + 3, hidden_nf=64, device=device, n_layers=4,
                    recurrent=True, norm_diff=args.norm_diff, tanh=args.tanh, learnable=False)
...
def train(model, optimizer, epoch, loader, backprop=True):
    ...
    for batch_idx, data in enumerate(loader):
        ...
        elif args.model == 'egnn_vel' or args.model == 'segno':
