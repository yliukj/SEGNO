...
from n_body_system.model import GNN, Baseline, Linear, EGNN_vel, Linear_dynamics, GMN, SEGNO
import os
...
def main():
    ...
    if args.model == 'gnn':
        model = GNN(input_dim=6, hidden_nf=args.nf, n_layers=args.n_layers, device=device, recurrent=True)
         ...
         model = SEGNO(in_node_nf=2, in_edge_nf=2 + 1, hidden_nf=args.nf, device=device, n_layers=args.n_layers,
    ...
    # pred = test_loss = train(model, optimizer, epoch, loader_test, backprop=False, save=True)
    # with open("prediction/" + f"{args.model}_pred.pkl", "wb") as outfile:
    #     pickle.dump(pred, outfile)

    return best_train_loss, best_val_loss, best_test_loss, best_epoch


def train(model, optimizer, epoch, loader, backprop=True, save=False):
    if backprop:
        model.train()
    else:
        model.eval()
    ...
    for batch_idx, data in enumerate(loader):
        ...
        elif args.model == 'gmn' or args.model == 'segno':
            nodes = torch.sqrt(torch.sum(vel ** 2, dim=1)).unsqueeze(1).detach()
            nodes = torch.cat((nodes, Z / Z.max()), dim=-1)
            rows, cols = edges
            loc_dist = torch.sum((loc[rows] - loc[cols])**2, 1).unsqueeze(1)  # relative distances among locations
        ...
        if save:
            pred.append(loc_pred.detach().cpu())
    ...
    if not save:
        ...
        return torch.concat(pred, dim=0)
